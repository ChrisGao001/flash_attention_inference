...
struct QuantizatioInt8PGBlockInfo {
    template <typename Params>
    __device__ QuantizatioInt8PGBlockInfo(const Params &params, const int bidb, const int seqlen_k)
        : b(bidb),
          s_k(seqlen_k),
    ...
    inline __device__ size_t k_scale_offset(const int row_stride, const size_t head_idx, const size_t head_stride,
                                            const size_t scale_idx) const {
        return static_cast<size_t>(sum_s_k + s_k) * row_stride + head_idx * head_stride + scale_idx;
    }
...
struct DecodingInt8PGBlockInfo {
    template <typename Params>
    __device__ DecodingInt8PGBlockInfo(const Params &params, const int bidb, const int bidh)
        : b(bidb),
          h(bidh),
    ...
    inline __device__ size_t k_scale_offset(const size_t seqlen_k, const size_t row_stride, const size_t head_stride,
                                            const size_t scale_idx) const {
        return static_cast<size_t>(sum_s_k + seqlen_k) * row_stride + h_k * head_stride + scale_idx;
    }
