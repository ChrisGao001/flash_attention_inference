...
inline void check_quantization_int8pg(const DecodingInt8PGParams &params, Tensor<half> *K, Tensor<half> *V,
                                      Tensor<int> *cu_seq_k) {
    ...
    size_t k_scale_elem_num = total_k * head_k * (dim / QUANTIZATION_GROUP_SIZE);
        ...
        for (size_t h = 0; h < head_k; ++h) {
            for (size_t sk = 0; sk < seq_k; ++sk) {
                for (size_t d = 0; d < dim; ++d) {
                    float k_scale =
                        __half2float(k_scale_ptr[(sum_seq_k + sk) * (head_k * (dim / QUANTIZATION_GROUP_SIZE)) +
                                                 h * (dim / QUANTIZATION_GROUP_SIZE) + d / QUANTIZATION_GROUP_SIZE]);
                    float v_scale =
                        __half2float(v_scale_ptr[(sum_seq_k + sk) * (head_k * (dim / QUANTIZATION_GROUP_SIZE)) +
                                                 h * (dim / QUANTIZATION_GROUP_SIZE) + d / QUANTIZATION_GROUP_SIZE]);
